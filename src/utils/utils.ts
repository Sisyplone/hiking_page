...
import { MAIN_COLOR, MUNICIPALITY_CITIES_ARR, NEED_FIX_MAP, RUN_TITLES, RIDE_TITLES, HIKE_TITLES,  ACTIVITY_TYPES, RICH_TITLE } from './const';
...
const titleForHike = (hike: Activity): string => {
  if (RICH_TITLE) {
    // 1. try to use user defined name
    if (hike.name != '') {
      return hike.name;
    ...
    // 2. try to use location+type if the location is available, eg. 'Shanghai Run'
    const { city, province } = locationForRun(hike);
    const activity_sport = getActivitySport(hike);
    if (city && city.length > 0 && activity_sport.length > 0) {
      return `${city} ${activity_sport}`;
    }
  }
  // 3. use time+length if location or type is not available
  const runDistance = hike.distance / 1000;
  const hikeHour = +hike.start_date_local.slice(11, 13);
  if (hikeHour >= 0 && hikeHour <= 10) {
    return HIKE_TITLES.MORNING_HIKE_TITLE;
  }
  if (hikeHour > 10 && hikeHour <= 14) {
    return HIKE_TITLES.MIDDAY_HIKE_TITLE;
  }
  if (hikeHour > 14 && hikeHour <= 18) {
    return HIKE_TITLES.AFTERNOON_HIKE_TITLE;
  }
  if (hikeHour > 18 && hikeHour <= 21) {
    return HIKE_TITLES.EVENING_HIKE_TITLE;
  }
  return HIKE_TITLES.NIGHT_HIKE_TITLE;
};
